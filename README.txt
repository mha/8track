...
Introduction:
	...
	playing patterns of other instruments (think Ableton Live session view).
...
Quick-start:
	...
	1. Install jRuby (http://jruby.codehaus.org/) and include it in your environment path
...
Interface:
	...
	The leftmost button starts and stops playback. If held for a moment, it will save the current patterns 
	to disk (two flashes of all buttons indicate the start and finish of the save operation).

	This row also shows the "running light" indicating the currently active step in a sequence.

	The default sequence length is the entire width of your Monome, but hold one of the steps in this row to set 
	the last step in a pattern (this does not affect the length of other patterns).